...
# # Streamlit app title
# st.title("Sara University ChatBot")
...
with tab1:
    ...
    if prompt := st.chat_input("How can I help you with?", key="tab1_chat_input"):
...
with st.sidebar:
    ...
    model_choice = st.selectbox(
        ...
        help="GPT-4: Broad, versatile | Llama 3: Fast, efficient"
    )
    ...
    if st.button("🔍Latest Chats - Langchain", help="Retrieve recent chats for LangChain"):
...
with tab2:
    ...
    if prompt := st.chat_input("How can I help you with?", key="tab2_chat_input"):
        # Check if the new query is the same as the previous one to avoid repetition
...
# Button to fetch recent chats for Tab 2
    ...
    if st.button("🔍 Latest Chats - LlamaIndex", help="Retrieve recent chats for Llamaindex"):
